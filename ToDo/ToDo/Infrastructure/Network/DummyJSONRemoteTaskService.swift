...
final class DummyJSONRemoteTaskService: RemoteTaskService {
    ...
    func fetchInitialTasks(completion: @escaping (Result<[TaskItem], Error>) -> Void) {
        guard let endpoint else {
            completion(seedTasksResult())
            return
        }

        session.dataTask(with: endpoint) { data, _, error in
            if let error {
                completion(self.seedTasksResult(fallbackError: error))
                return
            }

            guard let data else {
                completion(self.seedTasksResult())
                return
            }
    ...
    private func seedTasksResult(fallbackError: Error? = nil) -> Result<[TaskItem], Error> {
        do {
            let seedData = Data(Self.seedJSON.utf8)
            let tasks = try parseTasks(from: seedData)
            return .success(tasks)
        } catch {
            return .failure(fallbackError ?? error)
        }
    }
    ...
    // Fallback seed used when remote API is unavailable or returns empty list.
